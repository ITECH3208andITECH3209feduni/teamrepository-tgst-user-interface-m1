...
try{
    ...
    $handle = $link->prepare('select DISTINCT(tax_value), rate from sales'); 
    $handle->execute(); 
    $result = $handle->fetchAll(\PDO::FETCH_OBJ);
		
    foreach($result as $row){
        array_push($dataPoints, array("y"=> $row->rate, "label"=> $row->tax_value));
    }
	$link = null;
}
...
var chart = new CanvasJS.Chart("chartContainer", {
	...
	theme: "light1", 
	title:{
		text: "Study of Sales Taxable Value vs Sales Rate"
	},
	axisX:{
		title: "Taxable Value",
		suffix: " Rs: "
	...
	data: [{
		...
		markerType: "square",
		markerSize: 10,
		toolTipContent: "Rate: {y} percent<br>Taxable Value: {x} Rs: ",
